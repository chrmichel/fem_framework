...
public:
    ...
    DirichletBC(std::optional<Function> g_left,
                std::optional<Function> g_right)
        : g_left_(std::move(g_left)),
          g_right_(std::move(g_right))
    {
        if (!g_left_ && !g_right_)
            throw std::invalid_argument(
                "DirichletBC: at least one boundary function must be set.");
    }
               ...
               const core::Mesh<Dim>& mesh) const override
    {
        ...
        if constexpr (Dim == 1) {
            ...
            if (g_left_) {
                for (const std::size_t id : mesh.boundary_node_ids()) {
                    enforce_node(id, (*g_left_)(mesh.node(id)), A, b);
                }
            }
        }
    }

    std::string name() const override { return "DirichletBC"; }
